...
from fraudkit.features import build_features, fit_encoders, transform_with_encoders, CATEGORICAL, NUMERICAL
...
def main(config_path: str):
    ...
    train_pool = Pool(X_train[feature_cols], label=y_train, cat_features=cat_feature_indices)
    test_pool = Pool(X_test[feature_cols], label=y_test, cat_features=cat_feature_indices)
    ...
    # Save bundle
    bundle = ModelBundle(model=clf, encoders=encoders, feature_cols=feature_cols, threshold=threshold)
...
try:
    ...
    pool = Pool(Xs, label=ys, cat_features=cat_idx)
    ...
    import shap as _shap  # ensure dependency is present
    _ = _shap  # silence linter
